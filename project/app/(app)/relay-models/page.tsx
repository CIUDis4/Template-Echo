...
import { Plus, Search, Filter, ChevronLeft, ChevronRight, FileText, ExternalLink } from 'lucide-react';

interface RelayModelWithCount extends RelayModel {
...
export default function RelayModelsPage() {
  ...
  const [models, setModels] = useState<RelayModelWithCount[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [manufacturer, setManufacturer] = useState('All');
  const [status, setStatus] = useState('All');
  const [page, setPage] = useState(1);
  const [sortBy, setSortBy] = useState<'model_name' | 'feedback_count' | 'updated_at'>('model_name');
  ...
  const loadModels = async () => {
    ...
    const { data: feedbackData } = await supabase
      ...
      .select('relay_model_id');

    const countMap = new Map<string, number>();
    feedbackData?.forEach(f => {
      countMap.set(f.relay_model_id, (countMap.get(f.relay_model_id) || 0) + 1);
    });

    const enriched = (modelsData || []).map(m => ({
      ...m,
      feedback_count: countMap.get(m.id) || 0,
    }));
  ...
  const filtered = useMemo(() => {
    ...
    result = [...result].sort((a, b) => {
      ...
      if (sortBy === 'feedback_count') {
        valA = a.feedback_count;
        valB = b.feedback_count;
      }
      const cmp = valA < valB ? -1 : valA > valB ? 1 : 0;
      return sortDir === 'asc' ? cmp : -cmp;
    });
    return result;
  }, [models, search, manufacturer, status, sortBy, sortDir]);
  ...
  return (
    <div>
      <TopNav
        title="Relay Models"
        description={`${filtered.length} models${search || manufacturer !== 'All' || status !== 'All' ? ' (filtered)' : ''}`}
        actions={
          profile?.role === 'admin' && (
            <Link
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  ...
                  <th className="text-left px-4 py-3 font-medium text-muted-foreground">Template Version</th>
                  ...
                  paginated.map(model => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-xs text-muted-foreground">
                        {model.cloud_mod_date
                          ? new Date(model.cloud_mod_date).toLocaleDateString()
                          : '—'}
                      </td>
                      <td className="px-4 py-3">
                        <Link
